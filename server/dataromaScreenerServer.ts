...
async function handleUniverseStep(req: IncomingMessage, res: ServerResponse, sessionId: string): Promise<void> {
  let body = '';
  req.on('data', (chunk) => {
    body += chunk;
  });
  req.on('end', async () => {
    try {
      const payload = body ? (JSON.parse(body) as StepPayload) : {};
      const screener = await getOrchestrator();
